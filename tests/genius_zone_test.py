...
class GeniusZoneTests(unittest.TestCase):
    """
    ...
    def test_id_is_set_correctly_test(self):
        "Check that the id is set on the class"

        genius_zone = GeniusZone(self._device_id, self.raw_json, self.hub)
        self.assertEqual(genius_zone.id, self._device_id)

    def test_name_is_set_correctly(self):
        "Check that the name is set on the class"

        genius_zone = GeniusZone(self._device_id, self.raw_json, self.hub)
        self.assertEqual(genius_zone.name, self._zone_name)
